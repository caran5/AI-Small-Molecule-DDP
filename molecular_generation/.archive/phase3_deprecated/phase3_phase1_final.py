#!/usr/bin/env python3
"""
PHASE 3 PHASE 1 FINAL: RDKit Descriptors (Minimal Safe Set)
Using only well-tested descriptors
"""
...
print("="*80)
print("PHASE 3 PHASE 1 FINAL: RDKit Descriptors (Safe Set)")
print("="*80)
...
with tempfile.TemporaryDirectory() as tmpdir:
    ...
    for idx, (smiles_tuple,) in enumerate(molecules_raw):
        ...
        try:
            ...
            # NEW RDKit DESCRIPTORS (safe tested set)
            feat_rdkit = [
            ...
            # Combine
            feat_enhanced = feat_orig + feat_rdkit
            
            # Target
            logp = float(Descriptors.MolLogP(mol))
            
            X_original.append(feat_orig)
            ...
            error_count += 1
            if error_count <= 3:
                print(f"    Error {error_count}: {type(e).__name__}")
    
    print(f"  Final: {valid_count} valid, {error_count} errors")
    
    if len(X_original) < 50:
        print(f"❌ Too few valid molecules ({len(X_original)})")
        sys.exit(1)
    ...
    print(f"\n✅ Extracted {len(X_orig)} molecules")
    print(f"   Original: {X_orig.shape[1]}D (9 features)")
    print(f"   Enhanced: {X_enh.shape[1]}D ({X_enh.shape[1]-9} RDKit)")
    ...
    print(f"\nTEST 2: Enhanced Features ({X_enh.shape[1]}D Total)")
    ...
    if success_enh >= success_orig + 0.05:
        verdict = "✅ STRONG SUCCESS: RDKit descriptors provide +5% improvement"
        ...
        decision = "→ Feature quality insufficient; explore other approaches"
    ...
    results = {
        ...
        'models': {
            ...
            'enhanced_features': {
                'features': f'9 structural + {X_enh.shape[1]-9} RDKit (molecular weight, aromaticity, complexity, shape)',
                'rmse': float(rmse_enh),
                'mape': float(mape_enh),
